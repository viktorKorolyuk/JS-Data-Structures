		...
		input.forEach((a, index) => {
			...
			if (index === input.length - 1) {
				...
				b.forEach(e => {
					...
					// TODO: The for loop only works if the values after the object definition are non-object. I fixed it by returning out of the [add] function, but it still loops through it.
					for (var i = 0; i < e.split("}").length - 1; i++) {
						// Change scope
						scopeStack.pop();
						_obj = scopeStack[scopeStack.length - 1];
...
function tolerable(_input) {

	// If the input has X amount of spaces followed by quotation marks, its a string.
	if (/^ *"/.test(_input)) return eval(input);
	var inputTranslate = eval(_input);
